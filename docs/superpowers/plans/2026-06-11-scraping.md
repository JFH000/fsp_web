...
**Goal:** Build a one-time migration pipeline that scrapes ~200 product pages from fsparts.org (Zyrosite/JS-rendered), extracts structured data via Playwright + a local Qwen2.5-14B LLM, uploads images to Supabase Storage, and upserts all products into the Supabase `products` table.
...
**Tech Stack:** Python 3.11, uv, Playwright (Chromium), BeautifulSoup4, Qwen2.5-14B-Instruct via HuggingFace transformers 4.46.3 + bitsandbytes 4-bit quantization, torch 2.6.0+cu118, httpx, supabase-py, pytest.
...
No unit tests for model loading (requires GPU). The inspection notebook (Task 9) serves as the integration test.
...
def load_model(model_name: str = MODEL_NAME) -> tuple:
    """Download (first run) and load the model with 4-bit quantization."""
...
def extract_product_data(
    ...
    """Run the LLM on cleaned HTML and return parsed product dict, or None on failure."""
...
- [ ] **Step 2: Verify the file is importable (no GPU needed)**

```bash
cd scraping
uv run python -c "from fsparts_scraper.llm import parse_llm_response; print(parse_llm_response('{\"sku\": \"X\"}'))"
```
...
git commit -m "feat: add LLM module for structured product data extraction"
```
...
from fsparts_scraper.llm import load_model, extract_product_data
from fsparts_scraper.mapper import brand_lookup_text, product_line_lookup_text
import random, json
```
...
**Cell 3 — Load LLM (downloads model on first run, ~28GB):**
```python
model, tokenizer = load_model()
print("Model loaded on:", next(model.parameters()).device)
```

**Cell 4 — Inspect one product end-to-end:**
```python
...
print("\n=== LLM EXTRACTION ===")
data = extract_product_data(model, tokenizer, cleaned)
print(json.dumps(data, ensure_ascii=False, indent=2))
```

**Cell 5 — Run all 5 samples:**
```python
results = []
for url in sample:
    print(f"\n--- {url.split('/')[-1]} ---")
    ...
    prices = extract_prices(html)
    images = extract_images(html)
    llm_data = extract_product_data(model, tokenizer, clean_html(html))
    if llm_data:
...
**Cell 4 — Phase 2: Extract (skips already-checkpointed products):**
```python
errors = []
for url in tqdm(urls, desc="Extracting"):
    ...
    try:
        ...
        images = extract_images(html)
        cleaned = clean_html(html)
        llm_data = extract_product_data(model, tokenizer, cleaned)
