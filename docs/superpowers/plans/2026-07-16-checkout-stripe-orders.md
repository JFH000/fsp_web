...
- **Currency:** Stripe treats COP as a zero-decimal currency — `unit_amount` in a Checkout Session line item is the whole-peso integer directly, never multiplied by 100. This matches how `formatCurrency` already displays prices with `minimumFractionDigits: 0`.
